    ...
    public class Mode
    {
        ...
        public virtual bool TryActivate()
        {
            ...
            if (PlayingMode) //if is set to Toggle then if is already playing this mode then stop it
            {
                ...
                else if (newAbility.HasTransitionFrom && newAbility.Properties.TransitionFrom.Contains(ActiveAbility.Index)) //Means it can transition from one ability to another
                {
                    ...
                    ResetMode();
                    Animal.Mode_Stop(); //IMPORTANT  the Mode needs to be RESETED TO ENTER A NEW ABILITY
        ...
        public bool ForceActivate(int abilityIndex)
        {
            if (Animal.ActiveMode != null && !IsInTransition && !Animal.IsPreparingMode)
            {
                ...
                Animal.ActiveMode.ForceExit = true;
       ...
       // private int CurrentPlayingAnimation;
        ...
        {
            if (PlayingMode) return; // Means the mode is already playing. IMPORTANT.. works also when several modes have multiple Entering Animations

            if (ActiveAbility != null)
            {
                Animal.ActiveMode = this;

              //  CurrentPlayingAnimation = animatorTagHash;
                ...
                if (Animal.debugModes) Debug.Log($"Mode: <B>{Name}<color=Green> [ANIMATION ENTER] </color></B>  with Ability: <B> {ActiveAbility.Name} [{animatorTagHash}] </B> [{AMode}]  Time: " + Time.time.ToString("F3"));
        ...
        public void AnimationTagExit(int animationFullPath)
        {
          //  Debug.Log($"Mode: <B>{Name}<color=Green>");

            if (Animal.ActiveMode == this)               //Means that we just exiting the same animation
            {
                if (Animal.debugModes) Debug.Log($"Mode: <B>{Name}<color=red> [ANIMATION EXIT] </color></B> [{animationFullPath}] - [IgnoreExit - {ForceExit}] Time: " + Time.time.ToString("F3"));
        ...
        /// <summary> This is used to Force Modes to play </summary>
