...
namespace MalbersAnimations 
{
    public class ModifyStat : MonoBehaviour
    {
        ...
        public virtual void SetStats(GameObject go) { stats = go.GetComponentInParent<Stats>(); }
        ...
        public virtual void Modify(Component target)   { Modify(target.gameObject); }
    ...
    public class StatModifier
    {
        ...
        {
            if (stats != null && ID != null) ModifyStat(stats.Stat_Get(ID.ID));
        }

        /// <summary>Modify the Stats on an animal </summary>
        public void ModifyStat(Stat s)
        {
            if (s != null && modify != StatOption.None)
                switch (modify)
                {
        ...
        {
            if (stat == null) return;      //Means that there's no Stat
...
public class StatModifierDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        EditorGUI.PropertyField(line, ID, new GUIContent(string.Empty, "ID for the Stat to modify"));
        line.x += position.width / 3 +5;
        line.width += -10;
        EditorGUI.PropertyField(line, modify, new GUIContent(string.Empty, "Option to Modify"));
        line.x += position.width / 3 + 25;
        line.width -= 32;
        EditorGUI.PropertyField(line, Value, new GUIContent(string.Empty, "Value to Apply"));
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)   { return 16 * 2 + 2; }
