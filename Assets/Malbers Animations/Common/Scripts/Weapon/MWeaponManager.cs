...
namespace MalbersAnimations.HAP
{
    ...
    public class MWeaponManagerEditor : Editor
    {
        ...
        private SerializedProperty
             debug, LeftHandEquipPoint, RightHandEquipPoint, ForceMountNormalUpdate, UseDefaultIK, UseWeaponsOnlyWhileRiding,
            OnEquipWeapon, OnUnequipWeapon, OnCombatMode, OnWeaponAction, CombatLayerPath, CombatLayerName,
         StrafeOnTarget,  holsters, UseInventory, UseHolsters, DefaultHolster, HolsterTime,AlreadyInstantiated, StoreAfter,
          Editor_Tabs1, Editor_Tabs2, ForceNormalUpdate, m_WeaponAim, m_WeaponType, m_WeaponHand, m_WeaponHold, m_WeaponAction;//, ExitCombatOnDismount;
        ...
        private void DrawGeneral()
        {
           ...
           //EditorGUILayout.PropertyField(StrafeOnTarget, new GUIContent("Strafe on Target/Aim", "If is The Rider is aiming or has a Target:\nThe Character will be set to Strafe"));
            ...
            //EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            UseWeaponsOnlyWhileRiding.boolValue = GUILayout.Toggle(UseWeaponsOnlyWhileRiding.boolValue, new GUIContent("Use Weapons only while Riding", "The Weapons can only be used with the Riding System"), EditorStyles.miniButtonMid);
           // EditorGUILayout.PropertyField(UseWeaponsOnlyWhileRiding, new GUIContent("Only while riding", "Weapons can only be used while while the character is riding"));
            //EditorGUILayout.PropertyField(ExitCombatOnDismount, new GUIContent("Exit On Dismount", "Exit the Combat Mode on Dismount. meaning it will unequip weapons on Dismount"));
           // EditorGUILayout.EndVertical();
        ...
        private void DrawAdvanced()
        {
            //EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            //{
            //    EditorGUILayout.PropertyField(UseDefaultIK, new GUIContent("Use Default IK", "Uses Animator IK to Solve IK, Disable this if you are using another IK System"));
            //    EditorGUILayout.PropertyField(ForceNormalUpdate, new GUIContent("Force Anim Update R", "When the Rider is Aiming the 'Animator Update mode' will change to 'Normal', So I can Do IK Logic on Late UPDATE"));

            //    if (ForceNormalUpdate.boolValue)
            //    {
            //        EditorGUILayout.PropertyField(ForceMountNormalUpdate, new GUIContent("Force Anim Update M", "When the Rider is Aiming the 'Animator Update Mode' of the MOUNT Animal will also change to 'Normal'"));
            //    }
            //}
            //EditorGUILayout.EndVertical();
