...
namespace MalbersAnimations.Weapons
{
    ...
    public abstract class MWeapon : MDamager, IMWeapon, IAnimatorListener, IMDamager
    {
        ...
        ///// <summary>If left (-1) if Right(1)</summary>
        //public int HandInt => rightHand ? 1 : -1;
        ...
        public HolsterID HolsterAnim { get => holsterAnim; set => holsterAnim = value; }
        ...
        //    get
        //    {
        //        var dd = CanCharge ? ChargeCurve.Evaluate(Mathf.Clamp01(ChargeCurrentTime / ChargeTime)) : UnityEngine.Random.Range(0f, 1f);
        //        Debug.Log(dd);
        //        return dd;  
        //    }
        //}
       ...
       // public UnityEvent OnPlaced = new UnityEvent();
        ...
        {
            if (debug) Debug.Log($"{name}:<color=white> <b>[Main Attack Started] </b> </color>");  //Debug
        ...
        {
            if (gameObject.scene == null) return false; //Means is still a prefab
            WeaponOwner = _char;
            Owner = WeaponOwner.Owner;
            IsEquiped = true;
            CanAttack = true;
            ChargeCurrentTime = 0;
            ...
            if (debug) Debug.Log($"{name}:<color=white> <b>[Weapon Prepared] </b> </color>");  //Debug
            return true;
        }
        ...
        public virtual void ResetCharge()
        {
            if (CanCharge)
            {
                ...
                IsCharging = false;
                OnCharged.Invoke(0);
                if (debug) Debug.Log($"{name}:<color=white> <b>[Charge Reseted] </b> </color>");  //Debug
            }
        }
