...
namespace MalbersAnimations.HAP
{
    ...
    public partial class MWeaponManager
    {
        ...
        public void SetWeaponHand() => SetBoolParameter(Hash_WHand, Weapon.rightHand);
        ...
        public virtual void SetWeaponBeforeMounting(GameObject weapon)
        {
            ...
            Weapon = weapon.GetComponent<MWeapon>();
        ...
        public virtual void Equip_Weapon()
        {
            ...
            {
                ParentWeapon();
                StartCoroutine(MTools.AlignTransform(Weapon.transform, Weapon.PositionOffset, Weapon.RotationOffset, Vector3.one, HolsterTime)); //Smoothly put the weapon in the hand
            }
        ...
        public virtual void Draw_Weapon()
        {
            ...
            if (Weapon)
            {
                ...
                Action(DrawWeaponID);                          //Set the  Weapon Action to -1 to Draw Weapons From Right or from left -2

                if (Weapon.IsRightHanded) SendMessage(FreeRightHand,true); else SendMessage(FreeLeftHand,true);   //IK REINS STUFF

                if (debug) Debug.Log($"<b>{name}:<color=yellow> [Draw -> { (Weapon.IsRightHanded ? "Righ Hand" : "Left Hand")} -> {Weapon.Holster.name} -> {Weapon.name}]</color> </b>");  //Debug
            }
        }
        ...
        public void Equip_FAST()
        {
            ...
            if (Weapon.IsRightHanded) SendMessage(FreeRightHand, true); else SendMessage(FreeLeftHand, true);
